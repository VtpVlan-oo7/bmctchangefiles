...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    if (nBalance <= nReserveBalance || ((nBalance - nReserveBalance) <= 50 * COIN))  // SET THE MINIMUM TOKEN REQUIRE TO STAKE TO 50.
        return false;
    ...
    // Calculate coin age reward
    {
        ...
        if(txNew.nTime<GetTime()-86400)     // DEFINES THE 100% AND 50% STAKE REWARD DISTRIBUTION.
        nReward = Params().GetProofOfStakeReward(nCoinAge, nFees);
        else
        nReward = Params().GetProofOfStakeReward100(nCoinAge, nFees);
